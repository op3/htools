...
{
	return (float)sqrt(vector2f_dot(a_v, a_v));
}

struct Vector2f *
vector2f_mad(struct Vector2f *a_vo, struct Vector2f const *a_v0, struct
    Vector2f const *a_v1, struct Vector2f const *a_v2)
{
...
struct Vector2f	*
vector2f_negate(struct Vector2f *a_vo, struct Vector2f const *a_v)
{
	a_vo->x = -a_v->x;
	a_vo->y = -a_v->y;
	return a_vo;
